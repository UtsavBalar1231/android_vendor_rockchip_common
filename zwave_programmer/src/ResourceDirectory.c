    ...
    ZW_APPLICATION_TX_BUFFER *pCmd, WORD cmdLength, void* user)
{
      ...
      && n->state == STATUS_FIND_ENDPOINTS)
  {
    ...
    DBG_PRINTF("Nodeid: %d has: %d endpoints in this find report\n", n->nodeid, 
               n_end_points);
    ...
    WORD cmdLength, void* user)
{
      ...
      && n->state == STATUS_ENUMERATE_ENDPOINTS)
  {
    ...
    if (n->nEndpoints != n_end_points + 1)
    {
      ...
      n->nEndpoints = n_end_points+1; //Endpoint 0 is still there
    }
  }
  else
  {
...
GetCacheEntryFlag(nodeid_t nodeid)
{
  ...
  rc = n->security_flags;
  if (nodeid != n->nodeid) {
    ERR_PRINTF("Attempt to get security flag from a node entry with inconsistent node ID inside\n");
    assert(0);
  }
  rd_free_node_dbe(n);
  return rc;
...
int rd_check_security_for_unsolicited_dest( nodeid_t rnode,  security_scheme_t scheme, void *pData, u16_t bDatalen )
{
  ...
  BYTE *c = (BYTE *)pData;
  DBG_PRINTF("Check security for rnode:%u scheme %02x class %02x command %02x\n", rnode, scheme, pCmd->ZW_Common.cmdClass, pCmd->ZW_Common.cmd);
  ...
  DBG_PRINTF("highest_node_scheme. 0x%02x, node_scheme_mask: 0x%02x\n", highest_node_scheme, node_scheme_mask);
  ...
  if(CommandAnalyzerIsSupporting(pCmd->ZW_Common.cmdClass, pCmd->ZW_Common.cmd)) {
   ...
   /* Some non-secure legacy sensor devices send BASIC SET. We need to let these kind of BASIC SETs to unsol destination.
    * But we also need to make sure that supervision encapsulated BASIC Set are dropped and replied with NO_SUPPORT */
    /* So we allow BASIC CC if its send from non-secure node and which does not support Supervision */
