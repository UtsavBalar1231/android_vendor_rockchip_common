...
VersionHandler(zwave_connection_t *c, uint8_t* frame, uint16_t length)
{
  ...
  case VERSION_GET:
    {
      ZW_VERSION_REPORT_2BYTE_V2_FRAME* f = (ZW_VERSION_REPORT_2BYTE_V2_FRAME*) &txBuf;
      ...
      f->numberOfFirmwareTargets = ZIPGW_NUM_FW_TARGETS;
      ...
      ZW_SendDataZIP(c, (BYTE*) &txBuf, sizeof(ZW_VERSION_REPORT_2BYTE_V2_FRAME), NULL);
      break; //VERSION_GET
    }
  case VERSION_COMMAND_CLASS_GET:
    ...
    case COMMAND_CLASS_SECURITY:
      txBuf.ZW_VersionCommandClassReportFrame.commandClassVersion = is_sec0_key_granted() ? SECURITY_VERSION : 0x00;
      break;
    VERSION_CASE_LAN_ONLY(ZIP_ND)
#ifdef TEST_MULTICAST_TX
    VERSION_CASE2_LAN_ONLY(ZIP, V5)
#else
    VERSION_CASE2_LAN_ONLY(ZIP, V4)
#endif
  ...
  case VERSION_CAPABILITIES_GET:
      ...
      ZW_VERSION_CAPABILITIES_REPORT_V3_FRAME* fc = (ZW_VERSION_CAPABILITIES_REPORT_V3_FRAME*) &txBuf;
      fc->cmdClass = COMMAND_CLASS_VERSION;
      fc->cmd = VERSION_CAPABILITIES_REPORT;
      fc->properties = 0x07;
      ZW_SendDataZIP(c, (BYTE*) &txBuf, sizeof(ZW_VERSION_CAPABILITIES_REPORT_V3_FRAME), NULL);
    break;
  case VERSION_ZWAVE_SOFTWARE_GET:
      ...
      Get_SerialAPI_AppVersion(&av, &arv); /* Serial API Appl vers and revision. */
      ZW_Version((BYTE*) buf); /* Protocol version */

      ZW_VERSION_ZWAVE_SOFTWARE_REPORT_V3_FRAME* fz = (ZW_VERSION_ZWAVE_SOFTWARE_REPORT_V3_FRAME*) &txBuf;
      ...
      fz->hostInterfaceVersion1 = av;  // (MSB)
      fz->hostInterfaceVersion2 = arv; 
      fz->hostInterfaceVersion3 = 0;  // (LSB)
      ...
      ZW_SendDataZIP(c, (BYTE*) &txBuf, sizeof(ZW_VERSION_ZWAVE_SOFTWARE_REPORT_V3_FRAME), NULL);
   break;
  default:
...
REGISTER_HANDLER(VersionHandler, 0, COMMAND_CLASS_VERSION, VERSION_VERSION_V3, NET_SCHEME);
