...
/**
 * Query if a node is a virtual node
 *
 * Checks if the provided node id is registered as a virtual node.
 *
 * \note Uses an in-memory nodemask of virtual node. That nodemask is sync'ed
 * from the controller when calling copy_virtual_nodes_mask_from_controller().
 * I.e. that function must have been called prior to calling is_virtual_node().
 *
