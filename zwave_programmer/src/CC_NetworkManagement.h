...
typedef enum {
   ...
   * The NOP is sent before entering this state.  Now the gateway
   * waits for protocol callback nop_send_done() to trigger \ref
   * NM_EV_TX_DONE_SELF_DESTRUCT_RETRY.  Then we start a timer, call
   * ZW_RemoveFailedNode() and go to \ref
   * NM_WAIT_FOR_SELF_DESTRUCT_REMOVAL.
   */
   ...
   *
   * Used when zipgateway needs to probe because the cached info is
...
/**
 * Enable Smart Start mode if pending DSKs exist in provisioning list and Network Mangement is not busy.
 *
