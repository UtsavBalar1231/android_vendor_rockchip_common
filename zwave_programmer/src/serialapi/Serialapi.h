...
void
SerialAPI_ApplicationSlaveNodeInformation(uint16_t dstNode, BYTE listening,
...
/**
 * closes the NVM (restarts the radio) after read or write operation.
 * Returns:
 *  - 0x00: Success
 *  - 0x01: Generic error
...
/**
 * opens the NVM (stops the radio) before read or write operation.
...
/**
 * Checks if a command is supported by the current Serial API connection
 * 
...
/*
 * @brief Reports of Z-WaveLR is supported or not
 * 
 * @return true 
 * @return false 
 */
