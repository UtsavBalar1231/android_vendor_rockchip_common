...
BYTE
ZW_SetSlaveLearnMode(uint16_t node, BYTE mode,                            /*IN */
  VOID_CALLBACKFUNC(completedFunc)(BYTE, uint16_t, uint16_t))
{
  BYTE retVal;
...
/**
  ...
  * \param[in] parmLength  Number of Device parameter bytes
  */
void SerialAPI_ApplicationSlaveNodeInformation(uint16_t dstNode, BYTE listening, APPL_NODE_TYPE nodeType, BYTE *nodeParm, BYTE parmLength)
{
  int i;
  idx = 0;
  set_node_id_in_buffer(dstNode);
  buffer[ idx++ ] = listening;
  buffer[ idx++ ] = nodeType.generic;
  buffer[ idx++ ] = nodeType.specific;
  buffer[ idx++ ] = parmLength;
