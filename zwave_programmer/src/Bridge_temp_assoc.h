 ...
 * associations known to the gateway are defined with #MAX_TEMP_ASSOCIATIONS. If
 ...
 * gateway at the same time the oldest temp associations will be stolen/re-used
 * by the clients connecting last.
 *
 * \note In addition to the pool of #MAX_TEMP_ASSOCIATIONS temp associations a
 * global variable exist to handle a temp association with the ZIP client
 ...
 * a zero-second delay
 *
 * The reason for the zero delay is unclear - maybe it is to decouple the call
 * to temp_assoc_add_virtual_nodes from the current call-stack. Don't know
 * exactly - it's legacy functionality.
 */
void
 ...
 *
 * @param  was_dtls Was the request from the ZIP client received via DTLS?
 * @return pointer to temporary association
...
/**
 * Mark the temp association as locked by firmware update
 *
 ...
 * ensure that that association is actually also the one locked when this
 * function is called.
 *
