...
static bool connect_serial_api(const char* serial_device , SerialAPICapabilities_t* cap) {
  ...
  const char *sdk_ver = GenerateSdkVersionFromProtocolVersion((char*)zw_protocol_version);
  const char *nvm_id  = GenerateNvmIdFromSdkVersion(sdk_ver, cap->chip_library);

  printf(", Chip library: %d, ZW protocol: %s, SDK: %s, NVM: %s\n",
        cap->chip_library,
