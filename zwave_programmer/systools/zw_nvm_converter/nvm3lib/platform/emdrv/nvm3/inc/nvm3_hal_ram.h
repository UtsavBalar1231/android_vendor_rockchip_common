 ...
 *
 * @param[in] *bin_image
 *   Pointer to memory buffer with binary image. The data will be copied to the
 *   nvm3 RAM driver.
 * @param[in] image_size
 *   Size of binary image (must be less than or equal to the size passed to
 *   nvm3_halOpen())
 *
 * @return
...
/***************************************************************************//**
 * @brief
 *   Save copy of NVM3 RAM content to memory buffer
 *
 * @param[out] **nvm_buf_out
 *   Pointer to buffer pointer. This function will use malloc() to to allocate a
