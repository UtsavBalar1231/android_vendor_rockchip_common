...
static nvm3_file_descriptor_t nvm3_protocol_files_v0[] = {
  ...
  { .key = FILE_ID_NODEINFO, .size = FILE_SIZE_NODEINFO, .name = "NODEINFO",.optional=true, .num_keys = ZW_MAX_NODES},
  { .key = FILE_ID_NODEROUTECAHE, .size = FILE_SIZE_NODEROUTECAHE, .name = "NODEROUTECAHE",.optional=true, .num_keys = ZW_MAX_NODES},
  ...
  { .key = FILE_ID_CONTROLLERINFO, .size = FILE_SIZE_CONTROLLERINFO, .name = "CONTROLLERINFO"},
...
static nvm3_file_descriptor_t nvm3_protocol_files_v1[] = {
  { .key = FILE_ID_ZW_VERSION, .size = FILE_SIZE_ZW_VERSION, .name = "ZW_VERSION"},
  { .key = FILE_ID_NODEINFO_V1, .size = FILE_SIZE_NODEINFO_V1, .name = "NODEINFO_V1", .optional = true, .num_keys = ZW_MAX_NODES / NODEINFOS_PER_FILE},
  { .key = FILE_ID_NODEROUTECAHE_V1, .size = FILE_SIZE_NODEROUTECAHE_V1, .name = "NODEROUTECAHE_V1", .optional = true, .num_keys = ZW_MAX_NODES / NODEROUTECACHES_PER_FILE},
  { .key = FILE_ID_PREFERREDREPEATERS, .size = FILE_SIZE_PREFERREDREPEATERS, .name = "PREFERREDREPEATERS", .optional = true},
  { .key = FILE_ID_SUCNODELIST, .size = FILE_SIZE_SUCNODELIST, .name = "SUCNODELIST"},
  { .key = FILE_ID_CONTROLLERINFO, .size = FILE_SIZE_CONTROLLERINFO, .name = "CONTROLLERINFO"},
  { .key = FILE_ID_NODE_STORAGE_EXIST, .size = FILE_SIZE_NODE_STORAGE_EXIST, .name = "NODE_STORAGE_EXIST"},
...
{
  NODE_MASK_TYPE node_info_exists         = {};
  NODE_MASK_TYPE bridge_node_flags        = {};
  NODE_MASK_TYPE suc_pending_update_flags = {};
  NODE_MASK_TYPE pending_discovery_flags  = {};

  NODE_MASK_TYPE node_routecache_exists   = {};
  NODE_MASK_TYPE app_route_lock_flags     = {};
  NODE_MASK_TYPE route_slave_suc_flags    = {};
  ...
  for (uint8_t node_id = 0; node_id <= ZW_MAX_NODES; node_id++)
  {
...
/*****************************************************************************/
static json_object* controller_info_to_json(void)
{
  json_object* jo = json_object_new_object();
...
/*****************************************************************************/
json_object* controller_nvm_get_json(void)
{
  json_object* jo = json_object_new_object();

  json_object_object_add(jo, "backupInfo",   backup_info_to_json());
  json_object_object_add(jo, "zwController", controller_info_to_json());
...
/*****************************************************************************/
static bool parse_node_table_json(json_object *jo_ntbl, uint8_t ctrl_node_id, uint8_t *max_node_id)
{
  NODE_MASK_TYPE node_info_exists         = {};
  NODE_MASK_TYPE bridge_node_flags        = {};
  NODE_MASK_TYPE suc_pending_update_flags = {};
  ...
  *max_node_id = 0;
  ...
  for (int i = 0; (i < json_object_array_length(jo_ntbl)) && (i < ZW_MAX_NODES); i++)
  {
    json_object *jo_node = json_object_array_get_idx(jo_ntbl, i);
    uint8_t      node_id = json_get_int(jo_node,  "nodeId", 0, JSON_REQUIRED);
    ...
    if (node_id > 0)
    {
      ...
      ni->ControllerSucUpdateIndex = json_get_int(jo_node,  "controllerSucUpdateIndex", 0, JSON_REQUIRED);

      json_get_nodemask(jo_node, "neighbours", ni->neighboursInfo, JSON_OPTIONAL);

      json_object *jo_node_info;
      if (json_get_object_error_check(jo_node, "nodeInfo", &jo_node_info, json_type_object, JSON_REQUIRED))
      {
      ...
      if (json_get_object_error_check(jo_node, "routeCache", &jo_node_rc, json_type_object, JSON_REQUIRED))
      {
...
static bool parse_app_config_json(json_object *jo_app_config, uint8_t major_app_version)
{
  ...
  {
    ac.rfRegion = json_get_int(jo_app_config, "rfRegion", 0, JSON_REQUIRED);
    ac.iTxPower = json_get_int(jo_app_config, "txPower", 0,  JSON_REQUIRED);
    ac.ipower0dbmMeasured = json_get_int(jo_app_config, "power0dbmMeasured", 0, JSON_REQUIRED);
  }
...
/*****************************************************************************/
static bool parse_controller_json(json_object *jo_ctrl)
{
  SControllerInfo ci = {};
  SApplicationCmdClassInfo ai = {};
  SApplicationData ad = {};
  ...
  {
    if (false == parse_node_table_json(jo_node_table, ci.NodeID, &ci.MaxNodeId))
    {
...
bool controller_parse_json(json_object *jo, uint32_t protocol_version)
{
  ...
  if (false == is_json_file_supported(jo_ref, &major_prot_version, &major_app_version))
  {
    return false;
  }
  ...
  if (false == parse_controller_json(jo_ref))
  {
    return false;
  }
  ...
  {
    parse_app_config_json(jo_ref, major_app_version);
  }
