...
int main(int argc, char *const *argv)
{
  ...
  check_equal(rd_zgw_homeid_get(), rd_eeprom_homeid_get(),"HomeID does not match." );
  check_equal(rd_zgw_nodeid_get(), rd_eeprom_nodeid_get(),"NodeID does not match." );
  if(numErrs) {
    rc = 1;    
    goto exit;
  }
