 ...
 * mapped to fields in a running zipgateway, see \ref page-zgw-data.
 *
 * The functions for reading from these internal structures always give
 * pointers into the structure.  I.e., all data must be copied by the reader.
 *
 * The functions for adding data to the internal structures may steal or copy
...
/** \defgroup restore-db-zgw Restore Tool Internal Data for zipgateway
 * Data structures to store the runtime state of the zipgateway.
@{
 */
...
typedef struct zgw_node_IP_data {
   /** Length of #mDNS_node_name.  Derived from json string length. Initialize to 0. */
...
typedef struct zgw_ep_mDNS_data {
   /** mDNS field.  Must be valid according to SDS13782 before storing in this field. */
   char *endpoint_location;
   /** mDNS field.  Must be valid according to SDS13782 before storing in this field. */
   char *endpoint_name;
  /** Length of #endpoint_location.  Derived from json string length. Initialize to 0. */
   uint8_t endpoint_loc_len;
   /** Length of #endpoint_name. Derived from json string length. Initialize to 0. */
...
typedef struct zgw_node_ep_data {
   ...
   /** Length of #endpoint_info.  Computed from the composite of the
       relevant JSON fields. Initialize to 0. */
   uint8_t endpoint_info_len;
   /** Length of #endpoint_agg.  Derived from json hex-string
   ...
   /** End Point probing state.  Set to EP_STATE_PROBE_INFO by the
    ...
    * classes supported by the root device/end points.  Must be
...
/** Json data to configure zgw startup behaviour after restore.
...
typedef struct zgw_node_liveness_data {
   ...
   uint32_t lastUpdate; /**< Timestamp for the last successfull probe by ZGW. */
...
typedef struct zgw_node_probe_state {
    ...
    * node.
    *
    * If the state is set to DONE, though, it may be "down-graded" to
 ...
 * Z/IP Gateway type
 *
 * Data for a node that is a Z-Wave node, maintained by a Z-Wave
...
typedef struct zgw_node_data {
   ...
   zgw_node_uid_t node_uid; /**< (Locally) unique id, eg HomeID/nodeID
   ...
   zgw_node_pvs_t pvs_data; /**< Provisioning data. May be empty, eg,
...
typedef struct zip_lan_ip_data {
   /** ZipLanGw6 in cfg file.  Field gw_addr in \ref router_config.
    * IPv6 default gateway/next hop on the LAN side.  Initialize to
    * zeroes.  Required in cfg file for IPv6 connectivity. */
    ...
    * Generated ULA (stored in NVM area).  Initialized with zeroes.
...
typedef struct zw_security {
   ...
   /*S2 Network keys */
   uint8_t assigned_keys; /**< The gateways assigned keys.  Uses
...
/**
 * Access a writeable version of the data for the json reader to fill in.
 */
...
/**
 *  Get the gateway's data about itself and the network from the backup object.
 * \ingroup restore-db-zgw
 */
 ...
 * \ingroup restore-db-zgw
 */
const zgw_node_ep_data_t * zgw_node_ep_data_get(uint8_t nodeid, uint8_t epid);
...
/**
 * Check that the data read from import sources are complete, valid,
