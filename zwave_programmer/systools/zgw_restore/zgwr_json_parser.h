...
/** \defgroup json-reader ZGW Data File Reader for Restore Tool
 * \ingroup zgw-restore
 *
 * Parse a ZGW Data File, synchronizing the data with
 * zip_gateway_backup_data::controller, and collect the data in \ref
 ...
 *
 * Only critical errors will cause the parser to fail, eg, missing
 * critical data for the zipgateway itself.
 *
 * If there is a critical error in the description of another network
 * node, eg, missing nodeid, the node will be "dropped" by the parser.
 * When the zipgateway process starts up, it will see the node in the
