 ...
 * of a (stopped) zipgateway process (ZGW) by creating and populating
 * the ZGW storage files with state and network data.
 *
 * The Restore Tool uses a restored bridge controller and a
 * backup/migration zipgateway data file (\ref zgw_json_schema) to
 * generate the persistent runtime state of a zipgateway process.
 */
