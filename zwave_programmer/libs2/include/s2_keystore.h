 ...
 * This public key must be different from the secondary public key code returned by
 * \ref keystore_secondary_public_key_read . It must also match the private key returned
 * by \ref keystore_private_key_read.
 * \param[out] buf      Public key
 */
void keystore_public_key_read(uint8_t *buf);

 /**
  * Fetches the secondary public Curve25519 key from NVM and copies it to buf
  *
  * The secondary public key is used for unauthenticated key exchanges, where the
  ...
  * It must match the private key returned by \ref keystore_secondary_private_key_read .
  * It must be generated separately from the key fetched with \ref keystore_public_key_read .
  * \param[out] buf      Secondary public key
  */
 void keystore_secondary_public_key_read(uint8_t *buf);

/**
 * Fetches private Curve25519 key from NVM and copies it to buf .
 * It must be generated separately from the key returned by \ref keystore_secondary_private_key_read .
 * \param[out] buf      Private key
 */
void keystore_private_key_read(uint8_t *buf);

/**
 * Fetches the secondary private Curve25519 key from NVM and copies it to buf
 *
 * The secondary private key is used for unauthenticated key exchanges, where the
 * full public key (without blanking the initial 16 bits) is sent over the radio,
 * in order to protect the primary public key from a MITM attack as described in
 * PSIRT-33 and SWPROT-3265.
 * The returned private key must be generated separately from the one returned by
 * \ref keystore_private_key_read .
 * \param[out] buf      Secondary private key
 */
