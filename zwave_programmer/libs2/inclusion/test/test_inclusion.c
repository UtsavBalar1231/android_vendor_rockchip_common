...
void test_kex_joining_node_state_machine() {
  ...
  mock_call_expect(TO_STR(keystore_secondary_public_key_read), &p_keystore_read_pub_b);
  ...
  mock_call_expect(TO_STR(keystore_secondary_private_key_read), &p_keystore_read_priv_b);
  ...
  // When calculating the shared secret - Including node.
