 ...
 * are among the requested. Otherwise we use the secondary pair. This ensures that the primary keypair
 * will only ever be sent over the radio with two or more bytes zeroed out.
 * In CSA mode we also use the secondary keypair because we (the joining side) always send our full
...
static void s2_private_key_read(uint8_t *buf)
{
  ...
  {
    keystore_secondary_private_key_read(buf);
  }
}
...
static void s2_public_key_read(uint8_t *buf)
{
  ...
  {
    keystore_secondary_public_key_read(buf);
  }
}
