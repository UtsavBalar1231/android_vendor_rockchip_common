 ...
 */
static uint8_t
S2_send_data_both_cast(struct S2* p_context, const s2_connection_t* con, const uint8_t* buf, uint16_t len, event_t ev);
...
{
  CTX_DEF
  return S2_send_data_both_cast(ctxt, dst, buf, len, SEND_MSG);
}
      ...
      if (plain_text_len)
      {
        ...
        else
        {
          ...
          if (IS_LR_NODE(src->r_node)) {
            if(src->class_id == LR_ACCESS_KEY_SLOT) {
              src->class_id = ACCESS_KEY_SLOT;
            } else if(src->class_id == LR_AUTHENTICATED_KEY_SLOT) {
              src->class_id = AUTHENTICATED_KEY_SLOT;
            }
          }
#endif
...
S2_send_data_multicast(struct S2* p_context, const s2_connection_t* con, const uint8_t* buf, uint16_t len)
 {
  CTX_DEF
   return S2_send_data_both_cast(ctxt, con, buf, len, SEND_MULTICAST);
 }
...
#ifdef ZW_CONTROLLER
  if (IS_LR_NODE(con->r_node)) {
    convert_normal_to_lr_keyclass((s2_connection_t*)con); // dropping const so we can convert keyclass
  }
#endif
